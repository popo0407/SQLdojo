...
class SqliteLogHandler(BaseLogHandler):
    ...
    def get_logs(self, user_id: Optional[str] = None, limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 総件数を取得
                count_sql = "SELECT COUNT(*) as total FROM TOOL_LOG WHERE TOOL_NAME = 'SQLDOJOWEB'"
                count_params = []
                if user_id:
    ...
    def clear_logs(self, user_id: Optional[str] = None):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                if user_id:
                    conn.execute("DELETE FROM TOOL_LOG WHERE TOOL_NAME = 'SQLDOJOWEB' AND OPE_CODE = ?", (user_id,))
                else:
                    conn.execute("DELETE FROM TOOL_LOG WHERE TOOL_NAME = 'SQLDOJOWEB'")
                conn.commit()
        except Exception as e:
