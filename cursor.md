...
- 指示の分析と計画をした後に事前協議の手順に従い gemini と協議すること。
...
高リスク、中リスクの開発作業は以下の 7 ステッププロセスに従って実行すること：

1. **指示の分析と計画**: 要求を詳細に分析し、実装方針と影響範囲を明確化する
2. **事前協議**: gemini との協議により最適解を検討し、憲章適合性を確認する
   - 指示の分析と計画をした後に、以下の手順で gemini との協議を実施すること：
     1. gemini_consultation.md ファイルの内容を完全に削除（空ファイルにする）
     2. 新しい協議事項のみを記載（以下の構造で記載）：
        - 現在の状況
        - 改善要求
        - ユーザーに確認すべき不足情報
        - 検討すべき改善点
        - 実装方針の検討
        - 依頼事項
          Do 複数実装案提示して開発憲章に準拠した推奨提案依頼
          Don't 具体的なコードの提案と修正
     3. 新しいターミナルセッションを開始
     4. gemini -p "@gemini_consultation.md 　について日本語で回答をください。"で協議を実行
3. **方針決定**: gemini との協議により方針決定する
   - 事前協議を元に方針を検討して、以下の手順で gemini との協議を実施すること：
     1. gemini_decision.md ファイルの内容を完全に削除（空ファイルにする）
     2. 新しい協議事項のみを記載（以下の構造で記載）：
        - Cursor が考える改善方針と理由
        - 依頼事項
          Do Cursor が考える改善方針への意見
          Don't 具体的なコードの提案と修正
     3. 新しいターミナルセッションを開始
     4. gemini -p "@gemini_decision.md 　について日本語で回答をください。"で協議を実行
     5. 実装方針を決定
4. **段階的実装**: 小さな単位で実装し、各段階でテストを行う
5. **問題特定と修正**: テストで発見した問題を体系的に分析し、ログで検証して根本原因を特定して修正する
6. **動作テスト**: 実装完了後は必ずブラウザで実際の動作を確認する
