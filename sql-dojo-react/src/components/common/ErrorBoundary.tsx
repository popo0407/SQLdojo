...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (process.env.NODE_ENV === 'production') {
      this.sendErrorToLoggingService(error, errorInfo, errorId);
    }
  }
   ...
   * 外部ログサービスにエラーを送信
   */
  private sendErrorToLoggingService(error: Error, errorInfo: ErrorInfo, errorId: string) {
  ...
  private toggleErrorDetails = () => {
    const errorDetails = document.getElementById(`error-details-${this.state.errorId}`);
    if (errorDetails) {
