...
- **SQL 自動補完**: コンテキストに応じたキーワード、関数、データベースオブジェクトの補完
...
- **認証・ユーザー管理**: ログイン必須。ユーザー情報は user_data.json で管理し、管理者は DB から一括更新可能。
- **管理者機能**: 管理者認証（パスワード: mono0000）で管理者ページにアクセス。ユーザー情報・共通テンプレート・全 SQL 実行ログの管理が可能。
- **SQL 実行ログ**: 全 SQL 実行の履歴を最大 1000 件までファイル保存。ユーザー/管理者ごとに取得・クリア可能。
- **ユーザーページ**: SQL 実行履歴・個人テンプレート管理・SQL 実行ログの閲覧/クリアが可能。
...
# 新しい環境変数を追加
ADMIN_PASSWORD=mono0000  # 管理者認証用
USER_DATA_FILE=./user_data.json  # ユーザー情報ファイル
```
...
5. **SQL 補完機能の改善**
   ...
   - SELECT 句におけるカラム補完の精度向上
   - FROM 句におけるテーブル補完の精度向上
   - サブクエリや複雑な SQL 構造におけるコンテキスト認識の強化
...
- 認証・管理者認証・SQL 実行ログ・ユーザー/管理者ページ・API・テストを大幅強化
- SQL 実行ログは最大 1000 件ファイル保存、ユーザー/管理者で取得・クリア可能
- 管理者認証 API（パスワード: mono0000）・管理者ページ・ユーザーページ新設
- テスト用 user_data.json の切替・自動生成、pytest で全機能テスト
- .env に ADMIN_PASSWORD 追加
